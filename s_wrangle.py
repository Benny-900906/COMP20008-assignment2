...
def wrangle_school_locations(school_locations_file):
    ...
    school_data["Postal_Town"] = school_data["Postal_Town"].str.upper()
    #school_postcodes = school_data[["Postcode", "Postal_Town", "Address_Town"]]
    
    #temp = school_postcodes[school_postcodes['Address_Town'].str.upper() != school_postcodes['Postal_Town'].str.upper()]
    #temp.to_csv('temp.csv', index = False)
    
    
    school_location_dataframe = pd.DataFrame({
        "School Name": school_data["School_Name"],
        "School Type": school_data["School_Type"],
...
def wrangle_school_performance(school_performance_file):
    ...
    school_performance_dataframe = school_performance_dataframe.dropna()
    #school_performance_dataframe.to_csv('confirm.csv', index = False)
...
if __name__ == '__main__':
    ...
    final_data = final_data.merge(school_performance_data, how='left', on=['School Name']).dropna()
    final_data = final_data.drop('Locality_y', 1)
